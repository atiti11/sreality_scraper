...
public class EnricherLoader {
    ...
    public enum WriteResult { INSERTED, UPDATED, SKIPPED, ERROR }
    ...
    private void insertFactRow(String table, String propertyType, String dealType,
        ...
        switch (propertyType.toLowerCase()) {
            case "apartment"  -> insertApartment (table, dealType, doc, hashId, contentHash, geo, agencyId, validFrom, validTo, isActive);
            case "house"      -> insertHouse     (table, dealType, doc, hashId, contentHash, geo, agencyId, validFrom, validTo, isActive);
            case "land"       -> insertLand      (table, dealType, doc, hashId, contentHash, geo, agencyId, validFrom, validTo, isActive);
            case "commercial" -> insertCommercial(table, dealType, doc, hashId, contentHash, geo, agencyId, validFrom, validTo, isActive);
            default           -> insertOther     (table, dealType, doc, hashId, contentHash, geo, agencyId, validFrom, validTo, isActive);
        }
    }
    ...
    private int setCommon(PreparedStatement ps, int start, Document doc,
        ...
        if (validTo == null) ps.setNull(i++, Types.DATE); else ps.setDate(i++, Date.valueOf(validTo));
        if (geo == null) throw new SQLException(
            "Spatial join null for hash_id=" + hashId
            + " GPS=" + getDouble(doc,"gps_lat") + "," + getDouble(doc,"gps_lon"));
        ...
        if (lat == 0) ps.setNull(i++, Types.NUMERIC); else ps.setDouble(i++, lat);
        if (lon == 0) ps.setNull(i++, Types.NUMERIC); else ps.setDouble(i++, lon);
    ...
    private static Double computePricePerM2(long price, Integer areaSqm) {
        if (price == 0 || areaSqm == null || areaSqm == 0) return null;
        ...
        try (Connection c = pg.getConnection(); PreparedStatement ps = c.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                ...
                for (int col = 1; col <= meta.getColumnCount(); col++) {
                    ...
                    if (isInfraColumn(colName)) continue;
    ...
    private static Integer getFloorNumber(Document doc) {
        ...
        if (fc != null) {
            try { return Integer.parseInt(fc.split("/")[0].trim()); }
    ...
    private static Double parseAreaDouble(String raw) {
        if (raw == null) return null;
        try { return Double.parseDouble(raw.replace(" m²","").replace(" m2","").replace(",",".").trim()); }
    ...
    private static void setFirstSeenDate(PreparedStatement ps, int col, Document doc)
        ...
        catch (IllegalArgumentException e) { ps.setNull(col, Types.DATE); }
    }
